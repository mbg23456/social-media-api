...
def test_get_all_posts(authorised_client, test_posts):
    res = authorised_client.get("/posts/")  # Assuming this endpoint exists
    print(res.json())
    def validate(post):
...
def test_unauthorised_user_get_all_posts(client, test_posts):
    res = client.get("/posts/")  # Assuming this endpoint exists
    assert res.status_code == 401  # Unauthorized access

def test_unauthorised_user_get_one_post(client, test_posts):
    res = client.get(f"/posts/{test_posts[0].id}")  # Assuming this endpoint exists
    assert res.status_code == 401  # Unauthorized access

def test_get_one_post_not_existing(authorised_client, test_posts):
    res = authorised_client.get("/posts/999999")
...
def test_create_post_default_published(authorised_client, test_user, test_posts):
    res = authorised_client.post(
        "/posts/", json={"title": "arbitrary title", "content": "content would be here"})

    created_post = schemas.Post(**res.json())
    assert res.status_code == 201
    assert created_post.title == "arbitrary title"
    assert created_post.content == "content would be here"
